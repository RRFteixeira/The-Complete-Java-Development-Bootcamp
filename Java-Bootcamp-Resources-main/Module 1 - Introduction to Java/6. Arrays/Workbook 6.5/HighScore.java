public class HighScore {
    public static void main(String[] args) {
        ...
        int values[]= {randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber(), randomNumber()};
        int highScore = 0;

        for (int i= 0 ; i<values.length; i++){
            System.out.print(values[i] + " ");
            
            if (values[i]>highScore){
                highScore=values[i];
            }
        }
